...
void calculate_local_A(Matrix* A, float x1, float y1,float z1, float x2, float y2, float z2, float x3, float y3, float z3, float x4, float y4, float z4){
    A->set(((y3-y1)*(z4-z1)-(y4-y1)*(z3-z1)), 0, 0);   A->set(((-1)*(x3-x1)*(z4-z1)+(x4-x1)*(z3-z1)), 0, 1);   A->set(((x3-x1)*(y4-y1)-(x4-x1)*(y3-y1)), 0, 2);
    A->set(((-1)*(y2-y1)*(z4-z1)+(y4-y1)*(z2-z1)), 1, 0);  A->set(((x2-x1)*(z4-z1)-(x4-x1)*(z2-z1)), 1, 1);  A->set(((-1)*(x2-x1)*(y4-y1)+(x4-x1)*(y2-y1)), 1, 2);
...
void create_local_K(Matrix* K, short element_id, Mesh* M){
    ...
    /*
    Modificación de Parte 4:
    Se cambian los datos para la creacion de la K local por los obtenidos mediante la integración del modelo asignado
    */
...
void create_local_b(Vector* b, short element_id, Mesh* M){
    ...
    /*
    Modificación de Parte 4:
    Se cambian los datos para la creacion de la b local por los obtenidos mediante la integración del modelo asignado
    */
    b->set(J/90,0);
    b->set(J/90,1);
    b->set(J/40,2);
