/**
 * En lo que cambio con respecto al 2D es el sentido de que los métodos o funciones se deben adecuar a la nueva estructura del 
 * archivo generado por GID. 
*/
...
void read_input(string filename, Mesh* M){
    ...
    dat_file >> line; //quitamos "Coordinates"

    for(int i = 0; i < num_nodes; i++){
        short id;
        float x, y,z;//Obtenemos la corrdenada y. 
        dat_file >> id >> x >> y >> z;
    ...
    for(int i = 0; i < num_elements; i++){
        short id, node1_id, node2_id, node3_id, node4_id; //Cada elemento tiene 3 nodos 
